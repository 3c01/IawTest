...
float4 bump_spec_ps_main(VS_OUTPUT_SPEC In): COLOR
{
	...
	//float3 final_color = (diff + spec) * cloud_texel.rgb * fow_texel.rgb * (In.Pos2D.z/In.Pos2D.w);
	float3 final_color = {(In.Pos2D.z/In.Pos2D.w), (In.Pos2D.z/In.Pos2D.w), (In.Pos2D.z/In.Pos2D.w)};
...
VS_OUTPUT_SPEC sph_bump_spec_vs_main(VS_INPUT_MESH In)
{
	...
	float4x4 lightsView = look_at_matrix(m_light0ObjVector, {0.0f, 1.0f, 0.0f});
	float4x4 lightsProjection = ortho_matrix(-1000.0f, 1000.0f, -1000.0f, 1000.0f, -1000.0f, 1000.0f);

	float4x4 m_light0WorldViewProjection = lightsView * lightsProjection;

	Out.Pos2D = mul(In.Pos, m_light0WorldViewProjection);
